...
def search(blog_name):
    ...
    '''
    searched_bloges = search_blog(blog_name)
    title = f'search results for {blog_name}'

    return render_template('searchblog.html',bloges = searched_bloges)
...
def category(id):
    '''
    function that returns bloges based on the entered category id
    '''
    ...
    bloges_in_category = bloges.get_blog(id)
    return render_template('category.html' ,category= category, bloges= bloges_in_category)
